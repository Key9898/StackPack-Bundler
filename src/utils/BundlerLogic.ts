 ...
 */
export const replaceAssetUrlsInCSS = (
  cssContent: string,
 ...
 */
export const replaceAssetUrlsInHTML = (
  htmlContent: string,
  assetMap: Map<string, string>
): string => {
...
export const generateStandaloneHTML = async (
  ...
  const processedCSS = cssContents.map((css) => replaceAssetUrlsInCSS(css, assetMap)).join('\n\n');

  // Process HTML to replace asset URLs
  const processedHTMLContents = htmlContents.map((html) => replaceAssetUrlsInHTML(html, assetMap));

  // Wrap JS in IIFE
  const processedJS = jsContents.map((js) => wrapJSInIIFE(js)).join('\n\n');

  // Combine HTML (use first HTML file as base, or create default)
  const baseHTML =
    processedHTMLContents[0] || '<!DOCTYPE html><html><head></head><body></body></html>';
  ...
  const filename = customFileName && customFileName.trim() !== '' ? customFileName : 'bundle';
...
`;
  ...
  // Use custom filename if provided
  const filename = customFileName && customFileName.trim() !== '' ? customFileName : 'component';
